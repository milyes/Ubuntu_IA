...
const getCurrentPathStr = (path: string[]) => path.join('/') === 'home/ubuntu' ? '~' : `/${path.join('/')}`;
...
const TerminalLineItem = React.memo(({ 
  ...
  return (
    <div className="mb-2">
      {line.type === 'input' && (
        <div className="flex">
          <span className="text-[#87ff5f] font-bold mr-2 whitespace-nowrap">ubuntu@intelligence:{line.path}$</span>
...
const Terminal: React.FC<{ onAppOpen: (id: AppID) => void; addNotification: (text: string) => void }> = ({ onAppOpen, addNotification }) => {
  ...
  const [lines, setLines] = useState<TerminalLine[]>([
    { type: 'system', content: 'Bienvenue sur Ubuntu 24.04 LTS (GNU/Linux 6.8.0-generic x86_64)', path: '~' },
    { type: 'system', content: ' * Noyau IA:       Gemini 3 Flash', path: '~' },
    { type: 'system', content: ' * Capacités:      Ancrage Web Complet, Synthèse de Commandes', path: '~' },
    { type: 'system', content: '', path: '~' },
    { type: 'system', content: "Ubuntu Intelligence prêt. Tapez `help` pour les commandes ou `Tab` pour l'auto-complétion.", path: '~' },
    { type: 'system', content: '', path: '~' },
  ]);
  ...
  const simulateInstall = async (pkg: string, appId: AppID, manager: string) => {
    ...
    if (manager === 'snap') {
      steps = [
        { msg: `Download snap "${pkg}" from channel "stable"`, delay: 800 },
        { msg: `Fetch and check assertions for snap "${pkg}"`, delay: 400 },
        { msg: `Mount snap "${pkg}"`, delay: 300 },
        { msg: `Copy unit files of snap "${pkg}"`, delay: 300 },
        { msg: `Setup snap "${pkg}" security profiles`, delay: 500 },
        { msg: `Start snap "${pkg}" services`, delay: 400 },
        { msg: `${pkg} 2.4.0-ubuntu1 from Canonical✓ installed`, delay: 200 },
      ];
    } else {
      steps = [
  ...
  const handleCommand = useCallback(async (e: React.FormEvent) => {
    ...
    const currentPathStr = getCurrentPathStr(currentPath);
    setLines(prev => [...prev, { type: 'input', content: cmd, path: currentPathStr }]);
    ...
    try {
      // Built-in commands
      if (baseCmd === 'clear') {
        setLines([{ type: 'system', content: 'Console effacée. Ubuntu Intelligence en attente.', path: currentPathStr }]);
        return;
      }
...
- **Environnement** : Web-Integrated Ubuntu Emulator` }]);
      ...
      if (baseCmd === 'ai') {
        if (args.length === 0) {
          onAppOpen('ai-chat');
          setLines(prev => [...prev, { type: 'system', content: 'Ouverture de l\'interface de Chat IA...' }]);
          setIsProcessing(false);
          return;
        }
      ...
      if (response.functionCalls && response.functionCalls.length > 0) {
        ...
        for (const call of response.functionCalls) {
          ...
          setLines(prev => [...prev, { type: 'system', content: `[AI ACTION] Executing ${name}(${safeStringify(callArgs)}) -> ${String(result).substring(0, 50)}${String(result).length > 50 ? '...' : ''}` }]);
        }
      ...
      setLines(prev => [...prev, { type: 'error', content: `Erreur interne: ${err instanceof Error ? err.message : String(err)}` }]);
    } finally {
      setIsProcessing(false);
    }
  }, [inputValue, isProcessing, currentPath, fs, aiHistory, history, addNotification, onAppOpen]);
  ...
  }, [history, historyIndex, inputValue, suggestions]);
  ...
  const itemContent = useCallback((_index: number, line: TerminalLine, context: any) => {
    return <TerminalLineItem line={line} markdownComponents={context.markdownComponents} />;
  }, []);
      ...
      <div className="pt-2 shrink-0 border-t border-white/5 mt-2">
        ...
        <form onSubmit={handleCommand} className="flex items-center gap-2">
          <span className="text-[#87ff5f] font-bold mr-2 whitespace-nowrap">
            ubuntu@intelligence:{getCurrentPathStr(currentPath)}$
          </span>
          <input
            ref={inputRef}
            autoFocus
