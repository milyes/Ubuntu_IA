...
const Sidebar: React.FC<SidebarProps> = ({ onAppClick, onMenuClick, isMenuOpen, activeApp, icons }) => {
  const apps = [
  ...
  return (
    <div className="w-[68px] h-full flex flex-col items-center py-2 bg-black/30 backdrop-blur-lg border-r border-white/5 z-40">
      ...
      <div className="mt-auto mb-2 w-full flex justify-center border-t border-white/5 pt-2">
        <button 
          onClick={onMenuClick}
          className={`p-2 transition-all rounded-xl group relative ${isMenuOpen ? 'bg-[#E95420]/20' : 'hover:bg-white/10'}`}
          title="Afficher les applications"
        >
          <img 
            src="https://img.icons8.com/color/48/000000/ubuntu.png" 
            alt="Toutes les apps" 
            className={`w-10 h-10 transition-transform duration-300 ${isMenuOpen ? 'scale-110 rotate-[360deg]' : 'group-hover:scale-110 group-active:scale-95'}`} 
          />
          {isMenuOpen && (
            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-6 bg-[#E95420] rounded-r shadow-[0_0_8px_rgba(233,84,32,0.6)]" />
          )}
