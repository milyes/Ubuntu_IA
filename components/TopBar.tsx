...
const TopBar: React.FC<TopBarProps> = ({ onSearchClick, onChatClick }) => {
  ...
  return (
    <div className="h-7 w-full bg-black/40 backdrop-blur-md flex items-center justify-between px-4 text-[13px] font-medium z-50 select-none border-b border-white/5">
      ...
      <div className="flex items-center space-x-4">
        <button 
          onClick={onChatClick}
          className="flex items-center space-x-2 px-2 py-0.5 rounded hover:bg-white/10 transition-colors text-[#E95420]"
          title="Chat IA (Ctrl+I)"
        >
          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
            <path d="M12 2C6.48 2 2 6.48 2 12c0 1.54.36 2.98.97 4.29L1 23l6.71-1.97C9.02 21.64 10.46 22 12 22c5.52 0 10-4.48 10-10S17.52 2 12 2zm0 18c-1.47 0-2.84-.4-4.02-1.1l-.29-.17-4.38 1.29 1.32-4.54-.18-.3C3.75 14.01 3.33 12.55 3.33 11c0-4.78 3.89-8.67 8.67-8.67s8.67 3.89 8.67 8.67-3.89 8.67-8.67 8.67z" />
          </svg>
          <span className="text-[11px] font-bold">Chat IA</span>
        </button>
